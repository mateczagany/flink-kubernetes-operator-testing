...
public class BaseJob {
    ...
    protected DataStreamSource<TestData> source() {
        var kafkaSource = KafkaSource.<TestData>builder()
    ...
    protected KafkaSink<TestData> sink() {
        var builder = KafkaSink.<TestData>builder()
        ...
        if(parameters.has("kafka.transactional.id.prefix")) {
            builder = builder.setTransactionalIdPrefix(parameters.get("kafka.transactional.id.prefix"));
        }
